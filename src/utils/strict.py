...
Strict vision mode is enabled with REQUIRE_REAL_VISION=true. When active, scene
enrichment MUST use a real Qwen3-VL vision enricher on CUDA (location / actions /
visual_description / themes / mood must come from vision, not left unset).
...
STRICT_VISION_ENV = "REQUIRE_REAL_VISION"
...
def vision_strict_mode_enabled() -> bool:
    """Return True when strict real-vision (Qwen3-VL) mode is requested."""
    return os.getenv(STRICT_VISION_ENV, "false").lower() == "true"
...
def require_real_vision(provider, role: str = "SceneEnricher"):
    """Reject None or heuristic scene enrichers under REQUIRE_REAL_VISION=true.

    Returns the provider unchanged when it is a real vision enricher.
    """
    if not vision_strict_mode_enabled():
        return provider

    if provider is None:
        raise RuntimeError(
            f"{role} provider failed to load while REQUIRE_REAL_VISION=true. "
            "Vision scene enrichment aborted (no provider)."
        )

    name = getattr(provider, "name", "").lower()
    if name in ("heuristic", "mock", "unknown") or not name:
        raise RuntimeError(
            f"{role} resolved to {type(provider).__name__} while "
            "REQUIRE_REAL_VISION=true. Scene enrichment must use a real Qwen3-VL "
            "vision model, not heuristics. Aborting."
        )
    return provider